...
class IPU():
    ...
    def info_system(self):
        """Get system information from the PDU.

        Returns:
            dict: System information including model, firmware, MAC, etc.
                e.g. {'product_model': 'IP PDU', 'firmware_version': '1.0.6',
                      'mac_address': '00:04:A3:00:15:FE', 'system_name': 'IP PDU',
                      'administrator': 'admin', 'system_location': 'office'}
        """
        endpoint = self.endpoints['system']
        etree = self._api_request(endpoint)

        # Parse table rows for static values (model, firmware)
        rows = etree.xpath('.//table[1]//tr')
        info = {}

        for row in rows:
            cells = row.xpath('.//td')
            if len(cells) >= 2:
                key = '_'.join(''.join(cells[0].itertext()).strip().lower().split())
                value = ''.join(cells[1].itertext()).strip()
                if key and value:
                    info[key] = value

        # Parse input fields for editable values
        input_fields = {
            'mac': 'mac_address',
            'sysnm': 'system_name',
            'admin': 'administrator',
        ...
        for input_name, info_key in input_fields.items():
            element = etree.xpath(f'.//input[@name="{input_name}"]/@value')
            if element:
                info[info_key] = element[0]
